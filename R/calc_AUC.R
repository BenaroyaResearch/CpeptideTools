...
calc_AUC <-
  function(input_data, input_format = "wide",
    ...
    timepoints <-
      data.frame(
        timepoint_col =
          grep(pattern = paste0(pep_colname, "[_.-]?[0-9]+$"),
               x = colnames(input_data), value = TRUE) %>%
          as.character(),
      ...
      dplyr::mutate(
        ...
        timepoint_val =
          (timepoint_col %>%
             ...
             str_replace("[_.-]", "-") %>%
             as.numeric()))
    timepoints <- dplyr::arrange(timepoints, timepoint_val)
    
    # if 2-hour MMTT, drop all timepoints after 120
    if (auc_timeframe == "2-hour")
      timepoints <-
      dplyr::filter(timepoints, timepoint_val <= 120)
