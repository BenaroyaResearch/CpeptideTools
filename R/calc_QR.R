...
calc_QR <-
  function(
    ...
    assert_character(cpeptide_baseline_colname, len = 1, any.missing = FALSE)
    assert_character(cpeptide_timepoint_colname, len = 1, any.missing = FALSE)
    assert_character(age_colname, len = 1, any.missing = FALSE)
    assert_names(
      colnames(input_data),
      must.include = c(
